...
class HScript extends Iris
{
	...
	override function preset() {
		...
		#if sys
		set('Sys', Sys);
		set('System', lime.system.System);
		set('lime', {
			system: {
				System: lime.system.System
			}
		});
		#end
		...
		var parseJsonFunc = function(jsonString:String):Dynamic {
			...
			try {
				parsed = haxe.Json.parse(jsonString);
				return parsed;
			} catch(e1:Dynamic) {
				try {
					parsed = tjson.TJSON.parse(jsonString);
					return parsed;
				} catch(e2:Dynamic) {
					try {
						...
						try {
							if(Type.resolveClass('CoolUtil') != null) {
								var coolUtilClass = Type.resolveClass('CoolUtil');
								if(Reflect.hasField(coolUtilClass, 'parseJson')) {
									parsed = Reflect.callMethod(coolUtilClass, Reflect.field(coolUtilClass, 'parseJson'), [jsonString]);
									return parsed;
								}
							}
						} catch(e4:Dynamic) {
							trace('ERROR parseJson: No se pudo parsear JSON con ningun metodo disponible');
						}
					}
				}
			}
		...
		set('parseJson', parseJsonFunc);
		
		set('Json', {
			parse: function(str:String):Dynamic {
				return parseJsonFunc(str);
			}
		});
