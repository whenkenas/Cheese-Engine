...
class FlxFrame implements IFlxDestroyable
{
	...
	public function paint(?bmd:BitmapData, ?point:Point, mergeAlpha = false, disposeIfNotEqual = false):BitmapData
	{
		...
		{
			if(_point != null) _point.setTo(offset.x, offset.y);
			if (point != null)
	...
	public function subFrameTo(rect:FlxRect, ?frameToFill:FlxFrame):FlxFrame
	{
		...
		else
		{
			...
			frameToFill.offset.set(frameRect.x, frameRect.y).subtract(rect.x, rect.y).add(offset.x, offset.y);
	...
	public function clipTo(clip:FlxRect, ?clippedFrame:FlxFrame):FlxFrame
	{
		...
		else
		{
			...
			clippedFrame.offset.set(frameRect.x, frameRect.y).add(offset.x, offset.y);
