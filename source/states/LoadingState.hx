...
class LoadingState extends MusicBeatState
{
	...
	static function clearInvalidFrom(arr:Array<String>, prefix:String, ext:String, type:AssetType, ?parentFolder:String = null)
	{
		...
		while(i < arr.length)
		{
			...
			var isErectVariant:Bool = member.contains('/erect/') || member.startsWith('erect/');
			if(member.endsWith('/') || (!isErectVariant && !Paths.fileExists(myKey, type, false, parentFolder) && (doTrace = true)))
			{
				arr.remove(member);
